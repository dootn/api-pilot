...
export function UrlBar() {
  ...
  const { isWs, isSse, isMqtt, isGrpc, isDns, isRedis, isConnectionProtocol } = useProtocolMode(tab?.protocol);
  ...
  const { handleSend } = useProtocolHandlers(tab, updateTab, { isWs, isSse, isMqtt, isGrpc, isDns, isRedis });
  ...
  const handleUrlChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    } else if (lower.startsWith('redis://') || lower.startsWith('rediss://')) {
      if (!tab.protocol || tab.protocol === 'http') {
        updates.protocol = 'redis';
      }
  ...
  function buildSaveRequest() {
    ...
    return {
      ...
      name: tab!.isCustomNamed
        ? tab!.name
        : (tab!.protocol === 'websocket' || tab!.protocol === 'sse' || tab!.protocol === 'mqtt' || tab!.protocol === 'grpc' || tab!.protocol === 'dns' || tab!.protocol === 'redis'
                ? tab!.url
                : `${tab!.method} ${tab!.url}`),
  ...
  return (
    <div className="url-bar">
      ...
      <div style={{ display: 'flex', flex: 1, minWidth: 0 }}>
        {/* Method selector — hidden in WS, SSE, MQTT, gRPC, DNS, or Redis mode */}
        {!isConnectionProtocol && !isDns && !isRedis && (
          <MethodSelector
            method={tab.method}
      ...
      <button
        className={`send-btn ${(tab.loading || tab.wsStatus === 'connected' || tab.sseStatus === 'connected' || tab.mqttStatus === 'connected' || tab.grpcStatus === 'streaming' || tab.redisStatus === 'connected') ? 'cancel' : ''}`}
        onClick={handleSend}
        disabled={!tab.url.trim() && !tab.loading && tab.wsStatus !== 'connected' && tab.sseStatus !== 'connected' && tab.mqttStatus !== 'connected' && tab.grpcStatus !== 'streaming' && tab.redisStatus !== 'connected'}
      >
        {isWs
              ...
              : isGrpc
                ...
                : isRedis
                  ? tab.redisStatus === 'connected'
                    ? t('urlDisconnect')
                    : tab.redisStatus === 'connecting' || tab.loading
                      ? t('urlConnecting')
                      : t('urlConnect')
                  : isDns
                    ? tab.loading
