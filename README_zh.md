...
右键侧边栏中的任意集合 → **▶ 运行集合**，打开集合运行器弹窗。

- **选择请求**：选择全部或单个请求参与运行
- **配置选项**：设置**迭代次数**、**请求间延迟（ms）** 和**失败即停止**
- **持久化环境变量**：可选在运行完成后将 `pm.environment.set()` 的变量更改写回当前激活环境
- **实时进度**：每个请求的状态（✓ 通过 / ✗ 失败 / ⟳ 运行中 / ⚠ 错误）、HTTP 状态码、响应时间及测试通过/失败数实时更新
- **汇总栏**：运行完成后显示总请求数、测试通过/失败数和总耗时
...
- 在任意 HTTP 标签页上**右键**，选择 **⇄ 对比...** 即可打开对比面板
- **左右两侧各选一个标签页**，支持在所有有响应记录的标签页之间自由切换
- 四个子面板全面对比两次请求的差异：
  - **摘要** — 状态码、响应耗时、响应体大小；差异行自动高亮
  - **响应体** — 行级 LCS diff，JSON 自动美化后再比对；新增行标绿（+），删除行标红（−），顶部显示增删行数统计
  - **响应头** — 所有响应头键名并集表格，左右值不同或某侧缺失时行高亮
  - **请求** — Method + URL、Query 参数、请求头三段对比，差异行高亮
- 按 `Esc` 或点击遮罩关闭面板
